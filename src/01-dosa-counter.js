...
export function calculateDosaOrder(type, quantity = 1, isSpicy = false) {
  ...
  if(!type || typeof(type)!=='string' || !quantity || typeof(quantity)!=='number' ||
   Number.isNaN(quantity) || quantity<=0 || !prices.hasOwnProperty(type))
  {
    return null;
  }
  
  let pricePerDosa= prices[type]+(isSpicy?10:0);
